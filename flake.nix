{
  ...
  inputs = {
    ...
    nixpkgs-keybase-bumpversion.url = "github:mcdonc/nixpkgs/keybase-bumpversion";
      ...
      my_overlay = (
        self: super: {
          steam = super.steam.override {
            extraProfile = "export STEAM_EXTRA_COMPAT_TOOLS_PATHS='${nix-gaming.packages.${system}.proton-ge}'";
          };
          # prevent openssh from checking perms of ~/.ssh/config to appease vscode
          # https://github.com/nix-community/home-manager/issues/322
          openssh = super.openssh.overrideAttrs (old: {
            patches = (old.patches or [ ]) ++ [ ./patches/openssh-dontcheckconfigperms.patch ];
            doCheck = false;
          });
        }
      );
...

    {
      nixosConfigurations = {
        thinknix512 = nixpkgs.lib.nixosSystem {
          ...
          modules = shared-modules ++ [ ./hosts/thinknix512.nix ];
        };
        thinknix50 = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/thinknix50.nix ];
        };
        thinknix52 = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/thinknix52.nix ];
        };
        thinknix51 = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/thinknix51.nix ];
        };
        thinknix420 = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/thinknix420.nix ];
        };
        thinkcentre1 = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/thinkcentre1.nix ];
        };
        optinix = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/optinix.nix ];
        };
        nixcentre = nixpkgs.lib.nixosSystem {
          inherit system specialArgs;
          modules = shared-modules ++ [ ./hosts/nixcentre.nix ];
        };
